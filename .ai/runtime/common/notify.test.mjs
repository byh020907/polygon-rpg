...
test('UTF-8 JSON publication preserves server prefix, token and summary details', async (t) => {
  ...
  const env = {
    PGL_NTFY_URL: `http://127.0.0.1:${server.address().port}/prefix/product-loop`,
    PGL_NTFY_TOKEN: 'test-token',
  };
  assert.deepEqual(
    await notify(
      { ...input, blocker: '없음', nextAction: '결과 확인', commit: 'abc', sessionId: 'ses_1' },
      { env },
    ),
    { status: 'sent' },
  );
...
test('simultaneous calls serialize and durable ledger deduplicates', async (t) => {
  ...
  let calls = 0;
  const options = {
    env: { PGL_NTFY_URL: 'https://example.test/topic' },
    fetch: async () => {
      calls++;
      return { ok: true };
    },
...
test('disabled does not touch filesystem or network and unsafe URL is rejected', async () => {
  ...
  assert.equal(
    (
      await notify(input, {
        env: { PGL_NTFY_URL: 'https://name:secret@example.test/topic' },
        fetch,
      })
    ).status,
    'warning',
...
test('standalone help and JSON stdin work without dependencies', () => {
  ...
  const output = execFileSync(process.execPath, [cli, '--stdin'], {
    encoding: 'utf8',
    env: { ...process.env, PGL_NTFY_URL: '' },
    input: JSON.stringify({ event: 'blocked', key: '1', summary: '한글' }),
  });
  assert.deepEqual(JSON.parse(output), { status: 'disabled' });
  assert.throws(
    () => execFileSync(process.execPath, [cli, '--no-such-option'], { stdio: 'pipe' }),
    (error) => error.status === 2 && JSON.parse(error.stdout).status === 'usage_error',
  );
});
