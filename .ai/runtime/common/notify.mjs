...
function validate(input) {
  if (!input || !Object.hasOwn(events, input.event))
    throw new Error(`event must be ${Object.keys(events).join('|')}`);
  for (const name of ['key', 'summary'])
...
export async function notify(input, { env = process.env, fetch: send = globalThis.fetch } = {}) {
  ...
  try {
    ...
    if (
      ...
      throw new Error(
        'PGL_NTFY_URL must be an HTTP(S) topic URL without credentials, query or fragment',
      );
    ...
    while (!lock) {
      try {
        ...
        if (Date.now() >= deadline)
          throw new Error(
            `Notification lock busy: ${lockPath}. Retry later; after a crash remove this file only when its recorded PID has exited.`,
            { cause: error },
          );
        await sleep(50);
      }
    ...
    let sent = [];
    try {
      sent = JSON.parse(await readFile(ledger, 'utf8'));
    } catch (error) {
      if (error.code !== 'ENOENT') throw error;
    }
    if (!Array.isArray(sent)) throw new Error('Notification ledger must be an array');
    const id = createHash('sha256')
      .update(JSON.stringify([env.PGL_NTFY_URL, input.event, input.key]))
      .digest('hex');
    ...
    const response = await send(target.href, {
      ...
      headers: {
        ...
        ...(env.PGL_NTFY_TOKEN ? { Authorization: `Bearer ${env.PGL_NTFY_TOKEN}` } : {}),
      },
      body: JSON.stringify({
        topic,
        title: `${basename(repo)} · ${label}`,
        message,
        priority,
        tags: [tag],
      }),
...
async function cli(args) {
  ...
  try {
    ...
    for (let i = 0; i < args.length; i++) {
      const key = args[i];
      if (
        !['--event', '--key', '--summary', '--file', '--stdin', '--repo'].includes(key) ||
        Object.hasOwn(flags, key)
      )
        throw new Error(`Unknown or repeated option ${key}; use --help`);
      if (key === '--stdin') flags[key] = true;
      else {
        if (!args[i + 1] || args[i + 1].startsWith('--'))
          throw new Error(`Missing value for ${key}; use --help`);
        flags[key] = args[++i];
      }
    }
    const fromJson = flags['--file'] || flags['--stdin'];
    if (
      (flags['--file'] && flags['--stdin']) ||
      (fromJson && ['--event', '--key', '--summary'].some((key) => flags[key]))
    )
      throw new Error('Choose flags, --file, or --stdin; use --help');
    let input;
    if (fromJson) {
      let source = '';
    ...
    process.stdout.write(`${JSON.stringify({ status: 'usage_error', error: error.message })}\n`);
    process.exitCode = 2;
  }
}
