...
export const COMBAT_SKILL_UPGRADE_COSTS = Object.freeze([null, 1, 2, 3]);
...
export function getCombatSkillUpgradeCost(level) {
  if (!Number.isInteger(level) || level < 1 || level >= COMBAT_SKILL_UPGRADE_COSTS.length) {
    throw new RangeError('승급 대상 combat skill level은 1..3 사이의 정수여야 합니다.');
  }
  return COMBAT_SKILL_UPGRADE_COSTS[level];
}
