...
`Human Feedback Priority — PWA 릴리스 메타데이터·대기 업데이트 표시·저장 뒤 적용은 구현했지만, Codex in-app Browser가 persistent controller/site-data를 증명하지 못해 실제 지속 profile의 Release A → B 전환은 여전히 미검증이다.`
...
| PWA release update            | gap       | `v0.1.1`과 deterministic buildId가 메뉴와 Service Worker에서 같은 release metadata를 읽고 focused metadata/lifecycle fixture는 PASS. Codex in-app Browser의 새 localhost origin에서 A를 다시 열어 활성화를 기다린 뒤 B로 교체했지만 waiting/apply UI 없이 B가 직접 실행됐다. 지속 controller/site-data 상태를 증명하지 못했으므로 실제 persistent browser profile의 A → B waiting discovery·apply·offline B는 여전히 미검증이다. |
