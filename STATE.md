...
Human Feedback Priority — 실제 입력으로 보이는 무기·sweep·semantic hurt·damage 결과를 함께 판독할 수 있는 combat 검증 전선을 완성한다. 이번 선행 진단은 production pose/contact path → 실제 keyboard/touch input → hit/guard presentation 순서이며, 캠페인/후반 콘텐츠 확장은 이 전선과 승인 기반의 graphics reviewable data 안정화 전에는 재개하지 않는다.
