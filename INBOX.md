...
- 최신 공식 게임 위키·6슬롯 장비·세트와 특수 시너지 명세 및 실제 구현

  사용자 원문 (기존 시스템 작업 이후 장비 작업에 통합):

```text
아래내용 진행시켜 이미 진행중인 작업과 병행할건 병행하고 쌓을건 쌓아줘

Polygon RPG의 현재 기획과 구현을 아래 최신 Human Feedback 기준으로 정립해줘.

이번 작업은 반드시 두 단계로 진행한다.

1. repository에 장기적으로 남길 공식 게임 명세/위키 문서를 먼저 정리한다.
2. 그 문서를 authority로 삼아 현재 구현을 실제로 맞춘다.

문서만 수정하고 끝내지 않는다.
반대로 문서를 갱신하지 않은 채 코드만 수정하지 않는다.

기존 코드가 이미 어떤 방식으로 구현되어 있다는 사실은 기획 authority가 아니다.
다만 기존 Product Goal에서 이미 여러 차례 인터뷰로 확정된 스토리/캠페인 구조를 특별한 이유 없이 전면 재설계하지 않는다.

이번 최신 Human Feedback의 핵심 방향은:

> Polygon RPG는 레전드 오브 곡괭이의 기본 액션 RPG 시스템 문법을 baseline으로 삼고,
> 산업 수거·복구 세계관, 검/방패 역할, 장비 조합, 애니메이션/그래픽 품질,
> 5지역 자유 캠페인 등 Human Feedback이 있는 부분만 확장·변형한다.

레오곡에 이미 잘 작동하는 기본 RPG 시스템을 이유 없이 다시 발명하지 않는다.
...
- README.md
- docs/art-handoff/**
- docs/references/** 또는 현재 그래픽 reference 경로
- scripts/art-handoff-content.mjs
- scripts/generate-art-handoff.mjs
- scripts/art-production-contract.mjs
...
- src/graphics/**
- 장비/상점/도감/UI 관련 코드
- save/load/migration 경로

관련 scripts/tests 전체를 찾는다.
...
기존 unrelated 변경을 삭제하거나 되돌리지 않는다.
...
======================================================================
1-1. PRODUCT_GOAL.html을 "공식 게임 위키 + 제품 명세"로 정리
======================================================================

PRODUCT_GOAL은 단순 Requirement ID 목록만 나열하는 문서가 아니라
처음 보는 사용자/개발자/AI가 나무위키에서 게임 하나를 읽듯 훑어보고:
...
를 한 문서에서 이해할 수 있는 공식 게임 명세로 유지한다.

단, 구현 클래스명/Resolver/저장 schema 같은 내부 기술은 ARCHITECTURE로 보낸다.
...
======================================================================
1-2. 게임의 기본 Reference Baseline
======================================================================

게임 시스템 설계의 기본 출발점은 레전드 오브 곡괭이다.
...
Polygon RPG에서는 이것을 기반으로 Human Feedback이 있는 부분만 확장한다.
...
well-known한 우세안이 있거나
"기본 + override", "공용 + 고유", "자동 + authored 보정"이 명백한 경우
사소한 A/B/C 인터뷰를 반복하지 않는다.
...
사람들은 오래된 설비를 실제 생업에 사용하며,
수리하고 이어 붙이고 개조하면서 살아간다.
...
왜 주인공이 검/방패를 사용하는지 설명 대사로 직접 가르치지 않는다.
...
장비가 바뀌어도 이 기본 입력 문법을 가능한 한 유지한다.
...
- 검을 머리 위로 크게 드는 generic overhead windup 금지
- 몸 옆의 낮은 위치로 당김
- 골반과 흉곽이 먼저 회전
- 팔/손목이 뒤따름
- 빠른 횡/사선 베기
- Contact 이후 Follow-through
- 후반 감속
...
Gameplay가 공격 최대 범위를 먼저 정의한다.

실제 Hit 조건:

1. Visible Weapon Sweep이 적의 유효 Hurt Region에 실제로 닿음
AND
...
검 그림은 닿았지만 Gameplay 최대 범위 밖
→ MISS

Weapon Trail은 가능한 한 실제 Weapon Sweep trajectory에서 파생한다.

시각 궤적과 damage trace를 서로 독립된 임의 경로로 만들지 않는다.
...
전체 rectangle 하나를 기본으로 하지 않는다.

Visual SVG 전체 polygon을 그대로 collision으로 쓰지도 않는다.

Semantic Hurt Region:
...
약점/장갑/방패는 그래픽에서도 읽혀야 한다.

구르기 무적은 hurtbox 삭제가 아니라 gameplay invulnerability state다.
...
Polygon RPG 장비 슬롯은 다음을 공식 기준으로 한다.
...
레오곡의 무기/방어구/도구 구조를 기반으로 하되,
Polygon RPG에서는 방패를 독립 슬롯으로 확장한다.
...
문서/UI에서는 `offHand` 같은 개발 용어보다
"방패"라는 플레이어가 이해하기 쉬운 명칭을 우선한다.
...
주인공은 Field Cutter에 가장 익숙한 상태로 시작한다.
...
실제 출시 장비군 수는 아직 확정하지 않는다.

그러나 시스템은 최소 다음 유형을 받을 수 있어야 한다.
...
새 장비가 추가되어도
Basic / Strong / Guard / Roll 등의 기본 command grammar를 최대한 유지한다.
...
도구는 필드에서 비교적 빠르게 교체/사용 가능한 방향을 유지한다.

현재 구체적인 최종 도구 목록은 확정하지 않는다.


======================================================================
1-13. Field Capability
======================================================================

장비는 전투 스탯뿐 아니라 실제 현장 기능을 가진다.
...
필드 상호작용은 특정 Item ID가 아니라
Capability 의미로 정의한다.

장비를 단순한 던전 열쇠로 남발하지 않는다.

현재 기본 목적:

> 이 장비가 실제 현장 작업 장비라는 사실을
> 플레이를 통해 느끼게 하는 것.

강제 Metroidvania식 gating 여부는 별도 미정.


======================================================================
1-14. 장비 획득 / 성장 Baseline
======================================================================

레오곡 기본 구조를 baseline으로 한다.
...
반복 파밍 시스템을 특별한 이유 없이 과도하게 확장하지 않는다.
...
세트 시너지는 "같은 세트 장비"를 일정 수 이상 착용했을 때 발생한다.
...
특수 시너지는 세트 시너지와 완전히 다른 시스템이다.

정의:

> 서로 다른 장비/세트/계통을 교차 장착했을 때
> 의도된 특정 조합에서 발생하는 고유 시너지.
...
- Roll → Basic 연결 변화
- Strong 행동 변화
- Pull / Knockback 성질 추가
- Field Capability 결합

같은 플레이 방식 변화에 우선 가치를 둔다.

모든 가능한 조합을 자동 생성하지 않는다.

특수 시너지 조합은 authored catalog가 authority다.
...
특수 시너지는 텍스트 힌트 문구를 제공하지 않는다.

예:

"강한 반동을 받아낼 신발이 필요할 것 같다"

같은 직접 힌트 금지.

발견 규칙:

1. 관련 장비를 한 번도 소유한 적 없음
   → 관련 특수 시너지 존재를 굳이 노출하지 않음.

2. 관련 장비를 한 번이라도 보유한 적 있음
   → 장비/시너지 도감에 `???` 슬롯이 나타날 수 있음.
...
======================================================================
1-18. 성장 / Combat Skill
======================================================================

장비 성장과 플레이어 Combat Skill은 분리한다.

Equipment
= 어떻게 싸우는가

Combat Skill
= 같은 전투 문법에서 얼마나 숙련되었는가
...
등 기존 Combat Skill 개념을 유지한다.

향후 장비군별 별도 Skill Tree는 아직 확정하지 않는다.
...
라이벌 / 고물상인 / 지역 주민은 반복 등장하며
관계와 생활 변화가 누적되지만
게임 진행을 긴 대화가 자주 멈춰 세우지 않는다.
...
게임 화면 reference는
현재 대화에서 최종적으로 승인된
"실제 구현 가능한 플랫 항구 gameplay board" 수준의 단순도를 기준으로 한다.
...
캐릭터는 고퀄 일러스트가 아니라
실제 게임 화면에서 재현 가능한 플랫 벡터/폴리곤 형태로 만든다.
...
Codex가 중요 pose를 처음부터 임의 발명하고
"작동한다"는 이유로 완료하면 안 된다.

Key Pose reference가 authority다.


======================================================================
1-24. Authored Pose 범위
======================================================================
...
만 authored replacement 가능.

구르기/극단적 smear처럼
전체 silhouette가 크게 바뀌는 동작은
whole-body authored SVG 허용.
...
Shared Clip
→ Body Profile Retarget
→ Character Modifier
→ 필요한 순간 Contact IK
→ 중요 Pose Authored Override

IK를 모든 locomotion에 강제로 걸지 않는다.
...
Animation은 자연스러운 root curve를 가진다.

엔진이 Gameplay target에 맞춰
root movement를 warp한다.
...
Boss 연출은 별도 framing 가능.


======================================================================
1-29. 지역 Color Identity
======================================================================

전체 저채도/플랫 언어는 공유하지만
스크린샷만 봐도 지역을 구분할 수 있어야 한다.
...
Composition
= 그래픽 담당자가 승인한 장면 전체 배치.

Prefab
= 이미 예쁘게 조합된 중간 규모 자산.

Kit
= crate, lamp, ladder 같은 최소 반복 요소.

Codex가 Kit만 가지고
주요 장면을 처음부터 디자인하지 않는다.
...
Composition은 고정된 한 화면/두 화면 길이 단위가 아니다.
...
Composition은 additive/overlap 방식으로 이어져
플레이어에게는 연속 월드처럼 보인다.
...
Far/Mid/Foreground를 엔진의 고정 레이어로 만들지 않는다.
...
z
= 시각적 앞뒤 깊이 / 기본 parallax 기준

Gameplay는 현재 side-view X/Y 중심.

z는 우선 presentation authority.
...
진짜 perspective로 z가 멀다고 자동 축소하지 않는다.

기본 2D 구도를 유지하고
아트가 scale/parallax를 보정할 수 있다.


======================================================================
1-33. Landmark Presentation LOD
======================================================================

같은 Landmark를 Composition마다 별개 객체로 복제하지 않는다.
...
far는 단순히 detail hide만 하는 게 아니라
실루엣 보정 허용.
...
- 전투 중 즉시 무기 교체
- 장비별 별도 Skill Tree
- 강제 Field Capability gating
- Shield enchant
- 최종 특수 시너지 개수
- 모든 도구 종류

이미 확정된 것과 미정을 섞지 않는다.


======================================================================
1-39. ARCHITECTURE.md
======================================================================

PRODUCT_GOAL의 플레이어 규칙을 구현하기 위한
기술 계약만 남긴다.
...
- Slot
- Set Synergy
- Special Synergy Catalog
- Synergy Discovery State
- Field Capability
- Resolved Loadout
- Animation Profile
...
- Presentation LOD
- Master SVG export
- Lighting/Shadow authority

Product Goal 내용을 그대로 중복 복사하지 않는다.


======================================================================
1-40. docs/art-handoff/**
======================================================================

그래픽 담당자가 실제로 필요한 것만 남긴다.
...
- Field Cutter / Shield의 산업 장비 형태
- Composition / Prefab / Kit
- Landmark far/mid/near
- 지역 Color Identity

생성 문서는 generator/source authority를 수정한다.
...
현재 검+방패 Profile variant 구조를
실제 다중 슬롯 Equipment 시스템으로 이동한다.
...
repository naming convention에 맞춰
mainWeapon 등 다른 내부 명칭은 가능하다.

그러나 UI/Product 문서에서는:
...
과도한 OOP hierarchy는 만들지 않는다.


======================================================================
2-3. Equipment Family
======================================================================
...
를 저장/loadout/UI에 반영한다.

현재 방어구 콘텐츠가 부족하면
기본 placeholder/default item을 최소한으로 추가할 수 있다.

다만 새 방어구 효과/세트 수십 개를 임의 창작하지 않는다.
...
도구 교체/사용 UI가 이미 있다면 연결.

없다면 최소한:

- 장착 상태
- 장비 상세
- future quick-select contract

까지 구현.

대규모 새 도구 콘텐츠를 발명하지 않는다.
...
별도 Equipment Set Catalog를 만든다.

예:

{
  id: "shipyard-worker-set",
...
등을 조합할 수 있게 하되
현재 실제 필요 이상으로 범용 DSL을 만들지 않는다.

Special Synergy는 authored combinations만 존재한다.

자동 조합 생성 금지.


======================================================================
2-10. Special Synergy Discovery
======================================================================
...
관련 장비를 한 번도 보유하지 않음
→ 도감에 관련 synergy를 노출할 필요 없음.

관련 장비를 한 번 이상 보유
→ `???` 특수 시너지 슬롯 노출 가능.

실제 올바른 조합 장착
→ discoveredSpecialSynergyIds에 기록.

이후:
이름/조건/효과 영구 공개.

직접적인 문구 힌트는 생성하지 않는다.


======================================================================
2-11. Synergy Evaluation
======================================================================
...
Combat/Game/UI가 각 catalog를 따로 해석하지 않는다.

Special Synergy가 행동을 변경할 경우:

- command modifier
- attack modifier
- guard modifier
- field capability modifier

같은 명시적 hook을 사용.

GameScene에 synergy ID if문을 난립시키지 않는다.
...
특정 item ID를 story/scene에 hard-code하지 않는다.

기존 earliest playable flow에서
가능한 한 최소 한 곳 실제 연결.

새 mandatory progression gate는 임의로 만들지 않는다.


======================================================================
2-13. Progression / Save
======================================================================

현재 save schema를 확인하고
새 슬롯/시너지 발견 상태를 실제 저장한다.

기존 save migration 필요.
...
- 6개 장비 슬롯
- ever-owned history
- discovered special synergies

현재 schema version을 실제 변경에 맞춰 증가.
...
검 전용 swordEnchantments authority를
generic equipment enchant 구조로 일반화.

초기:
Field Cutter의 기존 enchant 동작 유지.

모든 슬롯이 enchant 가능하다고 자동 가정하지 않는다.

Shield/Armor enchant 신규 효과는
현재 기획에 없으면 만들지 않는다.
...
현재 검 profile ID 직접 참조를
Equipment Item ID로 이동.
...
새 장비 획득 방식을 불필요하게 복잡한
랜덤 loot 시스템으로 바꾸지 않는다.
...
기본 Field Cutter + Field Shield에서
리팩터링 전 전투 감각 유지.
...
Moveset → Animation Profile 연결.

기존 주요 공격/구르기는
Authored Pose를 받을 수 있는 구조 유지.

장비 Item마다 동일 animation 전체 복사 금지.
...
장비/방어구/도구가 Graphics Resource Catalog에
실제 Item 단위로 등록.
...
최종 승인되지 않은 고퀄 새 디자인을
Codex가 임의 확정하지 않는다.


======================================================================
2-19. Product Goal 위키 UI
======================================================================

현재 PRODUCT_GOAL.html을
사용자가 읽기 쉬운 위키형 구조로 재구성.
...
현재 Art Handoff는
최신 Product Goal Wiki를 참조하게 한다.

Reference 제작 시:

컨셉 일러스트보다
실제 게임 화면과 일치하는 reference를 우선.
...
- main weapon 장착
- shield 독립 장착
- helmet/body/boots 독립 장착
- tool 독립 장착
- incompatible hand usage
- two-hand future fixture

[전투]

- 기본 Cutter + Shield 결과 기존과 동일
- Guard / Just Guard / Counter
- visible weapon sweep
- gameplay envelope

[세트]

- 같은 Set ID piece count
- partial bonus
- full bonus
- UI 공개
...
- concrete item ID hard-code 없음

[그래픽]

- 모든 실제 장비 resource 등록
- review surface 정상
...
- 캐릭터 크기가 과도하게 커지지 않음
- 배경이 지나치게 복잡하지 않음
- 배경이 칙칙한 산업 폐허로 변하지 않음
- 플랫 디자인 유지
- 장비가 기사 판타지 장비처럼 튀지 않음
- Field Cutter/Shield가 현장 장비로 읽힘
- 지역 Color Identity
- 실제 gameplay scale에서 silhouette 읽힘

참조 수준은
현재 Human이 "딱이다"라고 승인한
단순한 플랫 gameplay reference 수준을 기준으로 한다.
...
작업 중 미정 사항이 생긴다고
모든 것을 Human에게 질문하지 않는다.

먼저:

1. 레오곡 baseline 확인
2. well-known 방식 검색
3. 현재 Product Goal 확인
4. 명백한 standard/hybrid면 합리적 기본안 적용
5. 게임 느낌/콘텐츠량/되돌리기 어려운 구조가 실제로 크게 달라질 때만 질문

질문이 필요하면:
실질적으로 다른 3안을 제시.

비슷한 두 안 + "둘 다" C를 반복하지 않는다.
...
- 실제 Breaker/Pole 완성 콘텐츠
- 수십 개 신규 방어구
- 수십 개 세트
- 수십 개 특수 시너지
- 랜덤 loot RPG 전환
- 대규모 Metroidvania gating
- 장비별 별도 Skill Tree
- 실시간 weapon swap 신규 전투 시스템
- 새 lore 설명 장문 대사
- 캠페인 전면 재설계
- 레오곡 baseline에 없는 시스템을 이유 없이 추가
...
[ ] PRODUCT_GOAL이 위키처럼 전체 게임 시스템을 한눈에 읽을 수 있다.

[ ] 레오곡 baseline과 Polygon RPG Delta가 혼동되지 않는다.

[ ] 주인공/라이벌/고물상인 역할이 최신 설정과 일치한다.

[ ] 장비 슬롯이 주무기/방패/투구/몸통/신발/도구로 실제 구현된다.

[ ] 기본 주무기는 Field Cutter다.

[ ] 기본 방패는 Field Shield다.
...
[ ] Set Synergy가 별도 시스템으로 동작한다.

[ ] Special Synergy가 교차 장비 조합으로 별도 동작한다.

[ ] Special Synergy 발견 전 `???` 도감 상태가 지원된다.

[ ] 관련 장비 보유 이력이 저장된다.

[ ] 발견한 Special Synergy가 저장/로드 후 유지된다.

[ ] 기존 검+방패 전투 체감이 회귀하지 않는다.

[ ] Combat Core가 concrete sword ID를 authority로 사용하지 않는다.

[ ] Visible Weapon / Attack Envelope 계약 유지.

[ ] Field Capability 계약이 구현된다.

[ ] 기존 save migration이 동작한다.

[ ] 기존 progression/forge/enchant/campaign 데이터 보존.

[ ] Graphics/Art Handoff가 최신 스타일을 반영한다.

[ ] 기존 테스트 통과.

[ ] 신규 장비/시너지/migration 테스트 통과.

[ ] npm run verify 통과.

[ ] 실제 gameplay Visual QA 통과.
...
1. 남긴/변경한 공식 문서
2. PRODUCT_GOAL 위키 구조
3. 세계관/주인공/라이벌 설정 변경
4. 장비 슬롯 구조
5. 기존 검 profile migration
6. Set Synergy 구현
7. Special Synergy + 도감 발견 구현
8. Combat 연결
9. Field Capability 연결
10. Progression / Save Migration
11. Forge / Enchantment
12. Graphics / UI
13. 실행한 테스트
14. Visual QA
15. 의도적으로 미구현한 미래 확장
...
문서만 그럴듯하게 만들어 놓은 것은 완료가 아니다.
구현 클래스만 추가해 놓은 것도 완료가 아니다.

새 게임을 실제로 시작했을 때
최신 Product Goal에 적힌 게임 규칙이
전투·장비·성장·저장·그래픽·UI에서 실제로 동작해야 완료다.
```
...
- 확정 제작 계약에 따른 시스템 우선 구현
  ...
  > 이기준으로 실제 시스템 구현진행해 디테일은 내가 그래픽리소스 받아올거야 시스템먼저
