...
- 폴리곤 전환의 문서·코드 근본 정합성

  사용자 원문:

  > 이건 근본정리니까 문서나 코드 꼼꼼히 정합해줘

  해석(사용자 원문과 구분): 기본 렌더러만 바꾸지 않고 폐기된 픽셀 후처리·설정·비교 UI·기본값·새 검증 산출물·테스트와 현재 문서의 모순까지 정리한다.

- 픽셀 효과 전면 제거와 폴리곤 표현 통일

  사용자 원문:

  > 지금 보니까 그냥 우리 픽셀 효과를 싹 제거 해버리자 그냥 폴리곤 형태 잘만드는게 퀄이나 디테일이 훨씬낫네

  해석(사용자 원문과 구분): 게임과 그래픽 검토실의 픽셀화·Retro 후처리와 그 설정/비교 경로를 제거하고 폴리곤 표현으로 통일한다. 본·부모 정규좌표·유형별 공용 애니메이션은 유지하면서 윤곽·명암·디테일의 품질을 폴리곤 원본에서 다룬다. 이전 픽셀 전용 표현 요구보다 이 최신 방향을 우선한다. 자동 루프 Human pause를 유지한다.
