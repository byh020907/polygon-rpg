...
- 레거시 버전 식별 실패 복구와 최신 배포 자동 적용 — 2026-09-12

  사용자 원문:

  ```text
Uncaught (in promise) Error: 현재 앱 버전을 확인하지 못해 업데이트를 보류합니다. 다시 확인해 주세요.

레거시 버전ㄷ일떄 이런게 있는데 그냥 항상 버전 확인안되면 최신 업데이트해야지
그리고 애초에 업데이트 확인형이 아니라 그냥 항상 최신 확인되면 업데이트로 해줘ㅗ
  ```

  처리 해석(사용자 원문과 구분): 현재 실행 중인 레거시 앱의 버전 metadata가 없거나 읽히지 않는 것을 최신 배포 설치·적용의 차단 조건으로 사용하지 않는다. 서버의 유효한 최신 release와 필요한 asset 준비를 확인할 수 있으면 현재 버전 미식별 상태에서도 갱신을 진행한다. 신규 버전 발견 후 수동 적용 버튼을 기다리는 기존 계약을 최신 배포 자동 준비·자동 적용으로 교체한다. 앱 시작·복귀·온라인 복귀와 제한된 주기 확인에서 동일 update owner가 처리하며, 준비된 최신 배포로 현재 진행 저장 성공 후 자동 전환한다. 현재 버전 식별 실패와 서버/네트워크/asset 준비 실패는 구분한다. 후자는 기존 정상 실행과 저장을 보존하고 재시도하며, 출처 불명의 배포나 불완전한 cache를 활성화하지 않는다. 저장·recovery를 삭제하거나 초기화하지 않으며 저장 실패 시 자동 reload하지 않는다. 중복 적용·무한 reload·mixed release·unhandled promise rejection을 방지하고 진행/실패 상태를 알린다. 이전의 수동 적용 요구보다 이번 Human 지시가 우선한다. 과거 PWA 완료 확인을 새 결함을 무시하는 근거로 쓰지 않는다. 자동 개발 루프의 Human pause는 유지한다.
