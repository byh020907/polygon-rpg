...
- 검토실·테스트 플레이 역할 분리와 원형 탐색 UX

  사용자 원문(발언 순서):

  ```text
렌더연구실이 있어서 오히려 헷갈린다 역할을 어케 나누는게 좋을까
그래 그렇게 하자
추가로 ux개선해줘
현재 멀 보려면 손이너무가
약간 선택하면 주변에 둥글게 나오는 식으로 하면 유형별 접근트리구조가 편할거같은데 레퍼런스 검색해서 이미지로 예시보여줘
  ```

  승인한 역할 분리(원문과 구분): 독립 렌더 연구실을 없애고 외형·본·애니메이션·프레임·조명은 그래픽 검토실에, 실제 조작·판정·카메라·지형 확인은 저장과 분리된 테스트 플레이에 둔다. 디버그 패널은 진입/장면 선택 입구가 된다. 재생 속도·본/메시·조명은 검토실의 진단 옵션으로 모은다.
  UX 해석(원문과 구분): 선택 위치 주변의 원형 메뉴와 유형 계층으로 반복 이동/조작 수를 줄인다. 큰 분류와 문맥 동작은 원형 메뉴, 많은 대상은 썸네일·검색으로 접근한다. Kando와 Maya marking menu의 실제 이미지 예시를 제시했다. 자동 루프 Human pause를 유지한다.
