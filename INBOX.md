...
- 그래픽 담당자 요청용 문서와 시나리오 정리

  사용자 원문:

  > 우리 문서좀 정리해줄래 전체 npc등 그래픽 리소스 목록과 해당 역할 표등 그리고ㅠ전체 시나리오랑
  >
  > 이걸하는 목적은 그에 맞게 그래픽 담당자에게 그래픽 요청하기 위해서임
  >
  > 기본적으로 product goal에 다 들어있는게 맞는데 이건 링크로 쪼개던가 하는방법이 나을거같기도해 컨텍스트가 클거라서
