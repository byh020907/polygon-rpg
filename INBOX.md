...
- PC에서 호환되지 않는 개발 저장의 초기화 진입 경로 누락

  사용자 원문:

  ```text
현재 버전 유지 · 저장하지 못해 새 버전을 적용하지 않았습니다 · 현재 캠페인과 호환되지 않는 개발 저장입니다. 새 게임으로 초기화하세요. &#x20;
pc는 초기화ㅂ튼이 없어
  ```

  해석(사용자 원문과 구분): 호환되지 않는 개발 저장 때문에 업데이트가 막혔을 때 PC에서도 찾을 수 있는 초기화/새 게임 복구 경로가 필요하다. Human이 중지한 자동 개발 루프는 재개하지 않는다.
