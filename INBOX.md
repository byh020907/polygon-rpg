...
- 전체 몹 교체 대신 유형별 대표와 공용 애니메이션 재사용 구조

  사용자 원문(발언 순서):

  ```text
지금 너무 다똑같은데 내가 컨셉 아트를 먼저 레퍼런스로 만들어서 줄테니 넌, 유형별로 대표 레퍼런스 한개만 만들어
짐승형, 비행형, 기타등등
그래픽 검토실에서 움직이는 대표 몹: 인간형·짐승형·비행형·기계형 각 1개
즉 내 의도는 컨셉 아트만 주면 본만 좀 바꾸고 유형별로 애니메이션 재활용해서 뽑아낼수 있게 하고싶은거야
  ```

  해석(사용자 원문과 구분): 현재 전체 몹을 비슷한 이족 형태로 일괄 교체하는 작업을 중단한다. 검토실에 인간형·짐승형·비행형·기계형 각각 움직이는 대표 1개만 만든다. 컨셉 아트는 Human이 제공할 예정이며, 같은 유형의 새 몹은 외형과 본 배치·비율만 바꾸고 유형별 공용 애니메이션을 재사용하도록 한다. 부모 축의 [-1,1] 로컬 정규좌표와 AI가 한 곳에서 편집하기 쉬운 구조가 우선이다. 주인공과 기존 실전 몹을 승인되지 않은 시안으로 일괄 대체하지 않는다.

- 몹 디자인의 부모 기준 정규좌표와 시안 비율 교정

  사용자 원문(발언 순서):

  ```text
그리고 데이터 구조도 일반적인 -1 1 사이 값으로 부모축 기준 로컬 정규좌표계 구조로 해줘 크기 상관없게
지금 나온 시안은 몸이 너무뚱뚱해
비율이 안맞아
  ```

  해석(사용자 원문과 구분): 디자인 원본은 부모 축의 로컬 [-1, 1] 정규좌표로 두고 크기·비율은 분리하여 부모 변경이 자식 부착물까지 전파되게 한다. 현재 몹 시안의 과도한 몸통 폭과 짧은 다리·작은 머리 조합은 승인되지 않았다. 주인공은 유지하며 몹의 몸통·허리·다리·머리 비율을 함께 교정한다.

- 렌더링 개선의 최우선 기준은 AI가 일관되게 쉽게 수정하는 구조

  사용자 원문(발언 순서):

  ```text
그리고 그거도 고려해줘, 현재 polygon to pixel 방식, 3d to 2d 방식이 근본적으로 디테일 표현하기 어려운 구조라면 다양한 개선 방법을 찾아줘 리소스 압박이 적은 방향으로
ㄴㄴ 내가 생각하는 가장 중요한점은 ai가 개발을 하되, 쉽게 수정이 가능한가 야

스프라이트 방식은 하나 바뀌면 다 다시 만들어야해서 이런방식을 안쓰는거고
  ```

  해석(사용자 원문과 구분): 렌더링 개선안은 AI가 원본 디자인을 한 곳에서 수정하면 모든 동작·검토실·실제 게임에 반영되는 편집성을 최우선으로 비교한다. 수정할 때마다 프레임 리소스를 다시 만들어야 하는 스프라이트 방식은 제외한다. 그 조건 안에서 CPU·메모리·용량 부담과 디테일 표현을 비교하며, 3D 본→2D 윤곽→정수 픽셀 경로의 실제 제약과 대안을 조사한다.
