...
for (const [name, width, height] of [
  ...
  try {
    ...
    await b.evaluate(
      `(async()=>{const [{createGameScene},{compileSvgMaster},{CanvasHost},{CanvasPolygonRenderer},{Camera2D}]=await Promise.all([import('/src/app/createGameScene.js'),import('/src/graphics/svg/SvgAssetCompiler.js'),import('/src/rendering/CanvasHost.js'),import('/src/rendering/CanvasPolygonRenderer.js'),import('/src/rendering/Camera2D.js')]);const asset=compileSvgMaster(${JSON.stringify(master)});const scene=createGameScene();scene.setCharacterAnimationSettings({svgAsset:asset,svgRootFrame:[-100,-100,300,220]});const canvas=document.createElement('canvas');canvas.style.cssText='position:fixed;inset:0;width:100vw;height:100vh;z-index:9999;background:#111';document.body.append(canvas);const host=new CanvasHost(canvas);host.resize();const renderer=new CanvasPolygonRenderer(host,new Camera2D());let samples=0;for(const id of ['idle','slash','heavy','shieldBash'])for(const progress of [0,.2,.4,.6,.8]){const g=scene.samplePlayerCombatGeometry({id,progress,phase:'active',sequence:5});const f=scene.createRenderFrame(1);renderer.render({...f,items:g.svgPresentation.items,scenePresentation:null,scenePresentationForView:null,cameraOffset:{x:0,y:0}});samples++;}window.__SVG_ACTOR_QA__={samples,frame:scene.createRenderFrame(1).combatGeometry.svgPresentation};scene.dispose();})();`,
    );
