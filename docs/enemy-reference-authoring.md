...
최우선은 **AI가 원본 한 곳을 바꾸면 모든 동작에 반영되는 편집성**이다. 3D 본→2D 폴리곤 자체가 동일한 몸체를 강제하지는 않는다. 픽셀화와 후처리는 사용하지 않는다. 유형마다 다른 본 계층과 외형을 갖게 하고, 가림과 부모 변환을 일관되게 적용한다. 작은 화면에서 읽히지 않는 세부는 면 크기·실루엣으로 정리한다.
