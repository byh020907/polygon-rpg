...
**찾기**를 눌러 원형 메뉴에서 주인공·몹·NPC·장비·월드·효과/UI로 접근한다. 몹 → 유형 견본에서는 인간형·짐승형·비행형·기계형으로 바로 이동한다. 많은 항목은 종류와 이름/ID 검색, 썸네일 목록으로 고른다. 세로 모바일은 미리보기를 먼저 보여 주며 **목록**으로 목록을 펼친다. 선택한 대상이나 미리보기를 누르면 주변에 재생·동작·장면·진단·테스트·복사 메뉴가 열린다. 가운데 버튼 또는 Escape로 한 단계 돌아가며 방향키와 Tab으로 이동한다. **개별 보기**와 **실제 장면 배치**, 폴리곤 출력의 실제 크기와 2×/4× 확대, 방향과 조명을 비교한다. 장면 리소스는 원래 배치와 광원을 사용한다. UI는 1280×720 또는 844×390의 실제 게임 컴포넌트를 표시하며 확대는 그 viewport를 유지한다.
