...
종류와 이름/ID 검색으로 리소스를 고른다. **개별 보기**와 **실제 장면 배치**, 폴리곤 출력의 실제 크기와 2×/4× 확대, 방향과 조명을 비교한다. 장면 리소스는 원래 배치와 광원을 사용한다. UI는 1280×720 또는 844×390의 실제 게임 컴포넌트를 표시하며 확대는 그 viewport를 유지한다.
