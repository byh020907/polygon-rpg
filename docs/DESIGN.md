...
- Developer parent의 candidate 뒤 같은 complete-work run 안에서 turn history를 상속하지 않는 read-only verifier subagent가 exact hash와 artifact를 독립 검증한다. FAIL은 parent가 수리해 새 verifier로 반복하고 PASS 뒤에만 통합한다.
