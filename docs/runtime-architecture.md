...
└─ GameScene Scene instance / root Node
   ...
   └─ Room Scene instance / root Node
      └─ TrainingEncounter Scene instance / root Node (resolved entity가 있을 때만)
```

- `GameApp`은 browser resource, input attach/detach, ResizeObserver, animation frame과 scene instance lifetime을 소유한다.
- `GameScene`은 persistent Player, camera presentation, 120Hz fixed-step combat·world state와 단일 read-only RenderFrame을 소유한다.
...
| Completion Signal   | `roomChanged`, `playerResultResolved`, `combatEventOccurred`             | Room/encounter가 이미 완료한 교체·판정 결과를 알리고 GameScene이 root writer로 적용 |
...
## Room / Training Encounter lifecycle

- `MapRuntime`이 active Room을 확정하면 GameScene이 resolved snapshot으로 fresh `Room` Scene을 attach하고 Room이 entity에 맞는 `TrainingEncounter` child를 조립한다.
